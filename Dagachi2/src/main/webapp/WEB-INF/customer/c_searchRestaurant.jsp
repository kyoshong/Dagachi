		...
		<div id="content_wrap3">
			...
			<div id="detailSearch" align="center">
				...
				<ul id="detail_search" class="example01" style="display: none;">
					<li>인원(단위:명)<input type="range" id=range name="range" value="1" min="1" max="10">
					<li>분위기 | 데이트<input type="checkbox" id="dating" name="dating" value="dating"> 
					가족<input type="checkbox" id="family" name="family" value="family"> 
					친구<input type="checkbox" id="friend" name="friend" value="friend"> 
					단체<input type="checkbox" id="groupReserv" name="groupReserv" value="groupReserv" checked="checked">
					<li>주차장<input type="checkbox" id="park" name="park" value="park">
					<li>유아놀이방<input type="checkbox" id="playGround" name="playGround" value="playGround">
				</ul>
			</div>

			<div id="content_wrap4">
				<div id="list">
					<div>"${list.count}"개의 지점이 있습니다</div>
					
					<c:forEach var="name" items="${list.restaurantInfoList}" begin="0" end="30" step="2">
					
							<a href="/customer/c_restaurantDetail?owner_Num=${name.owner_Num}" >
							<img style="height: 40%; width: 40%;" src="/resources<%=request.getContextPath()%>${name.restaurant_Pic}">${name.restaurant_Name}</a>
				...
				<div align="center">
					<c:if test="${list.count > 0}">
						<c:if test="${list.p.beginPageNumber > 10}">
							<a href="c_searchRestaurant?p=${list.p.beginPageNumber-1}">이전</a>
						</c:if>
						<c:forEach var="pno" begin="${list.p.beginPageNumber}"
							end="${list.p.endPageNumber}">
							<a href="c_searchRestaurant?p=${pno}">[${pno}]</a>
						</c:forEach>
						<c:if test="${list.p.endPageNumber < list.p.totalPageCount}">
							<a href="c_searchRestaurant?p=${list.p.endPageNumber + 1}">다음</a>
						</c:if>
					</c:if>
				</div>
			</div>
			<div id="side_wrap">
				<div id="map" style="width: 38%; height: 64vh;">
					<script async defer
						src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAyQalmVGgCv0luJDmeFzF31c7q2jQ-5Jg&callback=initMap&region=kr">
					</script>
					<script
