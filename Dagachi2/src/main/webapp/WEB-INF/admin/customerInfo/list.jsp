	...
	<center>
		...
		<c:if test="${list.count > 0}">
			<c:if test="${list.p.beginPageNumber > 10}">
				<a
					href="infoList?p=${list.p.beginPageNumber-1}${page.searchTypeKeyword}">이전</a>
			</c:if>
			<c:forEach var="pno" begin="${list.p.beginPageNumber}"
				end="${list.p.endPageNumber}">
				<span> <c:if test="${select != num}">
						<a href="infoList?p=${pno}${page.searchTypeKeyword}">[${pno}]</a>
					</c:if> 
			...
			<c:if test="${list.p.endPageNumber < list.p.totalPageCount}">
				<a href="infoList?p=${list.p.endPageNumber + 1}${page.searchTypeKeyword}">다음</a>
			</c:if>
		</c:if>
		</center>
</body>
